...
export default function ProductGallery({ images, title }: ProductGalleryProps) {
  ...
  return (
    <div className="flex flex-col gap-3 px-[clamp(16px,3vw,40px)]">
      <div className="relative aspect-square bg-[color:var(--color-plate)] overflow-hidden">
        <Image
          src={bust(gallery[active])}
          alt={`${title}${gallery.length > 1 ? ` — view ${active + 1}` : ""}`}
          fill
          sizes="(max-width: 768px) 100vw, 55vw"
          className="object-contain p-[7%] mt-product-img-trim"
          priority
        />
      </div>
            ...
            return (
              ...
              >
                <Image
                  ...
                  fill
                  sizes="120px"
                  className="object-contain p-[9%] mt-product-img-trim"
                />
              </button>
            );
          })}
